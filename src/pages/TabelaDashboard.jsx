...
export default function TabelaDashboard() {
  ...
  const metrics = useMemo(() => {
    ...
    return { totalGeral, totalPago, debito, contasVencendo, contasCount: contas.length };
  }, [contas, pagamentos]);
...
<div className=" text-center justify-center flex-col ">   <h1 className="  mb-5  text-3xl md:text-5xl text-white font-semibold text-center font-zalando ">{tabela?.nome || "Tabela"}</h1>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-primaria  p-4 rounded">
          <div className="text-sm text-gray-300">Total geral</div>
          <div className="text-2xl font-bold mt-1">{new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(metrics.totalGeral)}</div>
        </div>
        <div className="bg-black/40 p-4 rounded">
          <div className="text-sm text-gray-300">Total pago</div>
          <div className="text-2xl font-bold mt-1">{new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(metrics.totalPago)}</div>
        </div>
        <div className="bg-black/40 p-4 rounded">
          <div className="text-sm text-gray-300">Débito</div>
          <div className="text-2xl font-bold mt-1 text-red-500">{new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(metrics.debito)}</div>
        </div>
      </section>

      <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div className="bg-black/40 p-4 rounded">
          <h3 className="font-semibold mb-2">Pago x Pendente</h3>
